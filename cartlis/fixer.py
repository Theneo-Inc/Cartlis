    ...
    try:
        response = openai.ChatCompletion.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are an assistant that generates API content.",
                },
...
def fix_spec(api_spec, violations):
    for violation in violations:
        ...
        if not method:
            click.secho(
                f"Skipping violation because 'method' is missing: {violation}",
                fg="yellow",
            )
            continue
        ...
        if not path:
            click.secho(
                f"Skipping violation because 'path' is missing: {violation}",
                fg="yellow",
            )
            continue

        print(
            f"Applying fix for violation: {violation['id']} at {method.upper()} {path}"
        )
                ...
                api_spec["paths"][path][method]["security"] = [
                    {
                        "OAuth2": [
                            "write",
                            "read",
                        ]  # Array of strings instead of an object
                    }
                ]
        ...
        # SEC004: Add a default security scheme if missing
            ...
            if (
                violation["path"] in api_spec["paths"]
                and violation["method"] in api_spec["paths"][violation["path"]]
            ):
                if (
                    "security"
                    not in api_spec["paths"][violation["path"]][violation["method"]]
                ):
                    api_spec["paths"][violation["path"]][violation["method"]][
                        "security"
                    ] = [
                        {
                            "OAuth2": {
                    ...
                    print(
                        f"Added default security scheme to {violation['method'].upper()} {violation['path']}"
                    )
        ...
        # PERF001: Add rate limiting headers
            ...
            if (
                ...
                responses = api_spec["paths"][violation["path"]][
                    violation["method"]
                ].get("responses", {})
                if "200" in responses:
                    ...
                    print(
                        f"Added rate limiting headers to {violation['method'].upper()} {violation['path']} response 200"
                    )
        ...
        # PERF002: Add Cache-Control headers
            ...
            if (
                ...
                if "200" in responses:
                    ...
                    print(
                        f"Added Cache-Control headers to {violation['method'].upper()} {violation['path']} response 200"
                    )
        ...
        # CONSIST001: Convert parameter names to snake_case
            ...
            if (
                ...
                for param in api_spec["paths"][violation["path"]][
                    violation["method"]
                ].get("parameters", []):
                    if "name" in param:
                        ...
                        print(
                            f"Renamed parameter '{original_name}' to '{param['name']}' in {violation['method'].upper()} {violation['path']}"
                        )
        ...
        # CONSIST002: Convert schema properties to camelCase
            ...
            if violation["schema"] in api_spec["components"]["schemas"]:
                ...
                new_properties = {}
                for property_name, property_value in schema.get(
                    "properties", {}
                ).items():
                    new_name = to_camel_case(property_name)
                    new_properties[new_name] = property_value
                    print(
                        f"Renamed property '{property_name}' to '{new_name}' in schema {violation['schema']}"
                    )
            ...
            if violation["path"] in api_spec["paths"]:
                for method in api_spec["paths"][violation["path"]]:
                    ...
                    if "description" in api_spec["paths"][violation["path"]][method]:
                        api_spec["paths"][violation["path"]][method][
                            "description"
                        ] += "\nUse the following alternative: /v2/transactions."
                    print(
                        f"Marked {method.upper()} {violation['path']} as deprecated and added alternative."
                    )
        ...
        if violation["id"] == "DOC001":
            if (
                ...
                if not method_spec.get("description"):
                    method_spec["description"] = generate_ai_content(
                        f"Generate a description for {violation['method'].upper()} {violation['path']}"
                    )
                    print(
                        f"Generated description for {violation['method'].upper()} {violation['path']}"
                    )
        ...
        # OP_ID001: AI-Generated Operation ID
            ...
            if (
                ...
                if ai_operation_id:
                    ...
                    print(
                        f"Added AI-generated operationId to {violation['method'].upper()} {violation['path']}"
                    )
                else:
                    api_spec["paths"][violation["path"]][violation["method"]][
                        "operationId"
                    ] = f"operationId_placeholder_{violation['method']}"
                    print(
                        f"Added fallback placeholder operationId to {violation['method'].upper()} {violation['path']}"
                    )

        # PARAM001: Add missing path parameter definitions
        if (
            violation["id"] == "PARAM001"
            or violation["id"] == "UnresolvableParameterError"
        ):  # Catch other cases as well
            if (
                violation["path"] in api_spec["paths"]
                and violation["method"] in api_spec["paths"][violation["path"]]
            ):
                # Check if the path parameter is defined
                path_parameters = api_spec["paths"][violation["path"]][
                    violation["method"]
                ].get("parameters", [])
                param_names = [
                    param["name"] for param in path_parameters if param["in"] == "path"
                ]
                ...
                for param in missing_params:
                    if param not in param_names:
                        ...
                        print(
                            f"Added missing path parameter '{param}' to {violation['method'].upper()} {violation['path']}"
                        )
            ...
            if (
                ...
                for param in api_spec["paths"][violation["path"]][
                    ...
                    if not param.get("description"):
                        param["description"] = generate_ai_content(
                            f"Generate a description for the {param['name']} parameter in the {violation['method'].upper()} operation at {violation['path']}."
                        )
                        print(
                            f"Generated description for parameter '{param['name']}' in {violation['method'].upper()} {violation['path']}"
                        )
                ...
                if responses:
                    for status_code, response in responses.items():
                        ...
                        if "content" not in response:
                            ...
                            print(
                                f"Added content type to response {status_code} in {violation['method'].upper()} {violation['path']}"
                            )
                        ...
                        print(
                            f"Added headers to response {status_code} in {violation['method'].upper()} {violation['path']}"
                        )
        ...
        if "requestBody" in api_spec["paths"][path][method]:
            ...
            if request_body.get("description") is None:
                ...
                print(
                    f"Added default description to requestBody in {method.upper()} {path}"
                )
        ...
        # OP_SUM001: Add AI-generated summaries for operations lacking summaries
            ...
            if path in api_spec["paths"] and method in api_spec["paths"][path]:
                ...
                if not method_spec.get("summary"):
                    method_spec["summary"] = generate_ai_content(
                        f"Generate a concise summary for the {method.upper()} operation at {path}."
                    )
            ...
            if violation["schema"] in api_spec["components"]["schemas"]:
                ...
                for property_name, property_value in schema.get(
                    ...
                    if "description" not in property_value:
                        property_value["description"] = generate_ai_content(
                            f"Generate a description for the '{property_name}' field in the {violation['schema']} schema."
                        )
                        print(
                            f"Generated AI description for schema field '{property_name}' in {violation['schema']}"
                        )
            ...
            for method, details in methods.items():
                ...
                for response_code, response in responses.items():
                    if response is None:
                        ...
                        print(
                            f"Fixing None response for status code {response_code} in {method.upper()} {path}"
                        )
                        responses[response_code] = {"description": "Default response"}
        ...
        for violation in violations:
            ...
            if not path:
                click.secho(
                    f"Skipping violation due to missing path: {violation}", fg="yellow"
                )
                continue

            if not method:
                click.secho(
                    f"Skipping violation due to missing method: {violation}",
                    fg="yellow",
                )
                continue
                ...
                if violation["id"] == "RESP001":
                    ...
                    for status_code, response in responses.items():
                        if response is None:
                            # Replace None response with a default response object
                            print(
                                f"Fixing None response for status code {status_code} in {method.upper()} {path}"
                            )
                            responses[status_code] = {"description": "Default response"}
            else:
                print(
                    f"Path {path} or method {method} no longer exists after versioning fix"
                )
